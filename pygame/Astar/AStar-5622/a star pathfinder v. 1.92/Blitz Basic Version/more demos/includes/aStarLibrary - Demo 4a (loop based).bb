...
Function CheckPathStepAdvance(unit.unit)
	If (unit\xLoc = unit\xPath And unit\yLoc = unit\yPath) Or unit\pathLocation = 0		
		...
		Else If unit\pathLocation < unit\pathLength 		
...
;This function claims nodes for a unit. It is called by ReadPath()
Function ClaimNodes(unit.unit)
	...
	;If no collision is detected, claim the node
	...
	Else		
		...
		Else If UnitOnOtherUnitPath(unit.unit,unit\unitCollidingWith.unit)
			unit\pathStatus = FindPath(unit.unit,unit\targetX,unit\targetY)	
...
Function DetectCollision.unit(unit.unit)			
	...
	If claimedNode(x2,y2) = Null
		...
		If x1<>x2 And y1<>y2 ;if next path step in diagonal
...
Function ReachedNextPathNode(unit.unit)
	If unit\pathStatus <> found Then Return True	
	If unit\xLoc = unit\xPath And unit\yLoc = unit\yPath Then Return True	
	Return False
End Function
