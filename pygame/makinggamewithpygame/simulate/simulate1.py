...
def main():
    ...
    while True:
        ...
        surface.blit(scoresurf,scorerect) # blit(source,dest,...) : draw one image onto another
        ...
        for e in pygame.event.get():
            if e.type == MOUSEBUTTONUP:
                ...
                elif e.key == K_s: clickedbutton = green # print 'clicked : ' ,clickedbutton
        if not waitingforinput:
            ...
            if clickedbutton and clickedbutton == pattern[currentstep]:
                # pushed the correct button #print 'pushed the correct button'
                flashButtonAnimation(clickedbutton)
                currentstep += 1
                lastclicktime = time.time()
                if currentstep == len(pattern): # pushed last button in the pattern
                    changeBackgroundAnimation()
                    score += 1
                    waitingforinput = False
                    currentstep = 0 # reset back to first step
            elif (clickedbutton and clickedbutton!=pattern[currentstep]) or (currentstep!=0 and time.time() - timeout > lastclicktime):
                print 'pushed the incorrect button'
...
def changeBackgroundAnimation(animationspeed=40):
    global bgcolor
    newbgcolor = (random.randint(0,55),random.randint(0,55),random.randint(0,55))
    newbgsurf = pygame.Surface((width,height))
    newbgsurf = newbgsurf.convert_alpha()
    r,g,b = newbgcolor
    for alpha in range(0,55,animationspeed):
        checkForQuit()
        surface.fill(bgcolor)
