...
def runGame():
    ...
    while True:
        ...
        for e in pygame.event.get():
            if e.type == KEYUP:
                if e.key == K_p: # pasuing the game
                    displaysurf.fill(bgcolor)
                    pygame.mixer.music.stop()
                    showTextScreen('Paused') # until a key press
                    pygame.mixer.music.play(-1,0.0)
                ...
                if (e.key == K_LEFT or e.key == K_a) and isValidPosition(board,fallingpiece,adjX=-1):
                    fallingpiece['x'] += -1
                    movingleft = True
                    movingright = False
                    lastmovesidewaystime = time.time()
                elif (e.key == K_RIGHT or e.key == K_s) and isValidPosition(board,fallingpiece,adjX=1):
                    fallingpiece['x'] += +1
                    movingleft = True
                    movingright = False
                    lastmovesidewaystime = time.time()
                # rotate the piece if there is room to rotate
                elif (e.key == K_UP or e.key == K_w) and isValidPosition(board,fallingpiece,adjX=1):
...
def drawPiece(piece,pixelx=None,pixely=None):
    ...
    if (pixelx,pixely) == (None,None):
        pixelx,pixely = converToPixelCoords(pixelx['x'],piece['y'])
