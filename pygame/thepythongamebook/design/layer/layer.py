...
class Text(pygame.sprite.Sprite):
    ...
    def newMsg(self, msg='New message'):
        ...
        self.rect.center = self.app.get_width()/2,10
...
class Mountain(pygame.sprite.Sprite):
    def __init__(self,app,atype):
        ...
        def setupImage(self):
            w = 100 * self.type * 1.5
            h = app.screen.get_height()/2 + 50*(self.type-1)
            self.image = pygame.Surface( (w,h) )
            self.image.set_colorkey(black)
            pointlist = (0,h),(0,h-10*self.type),(w/2, int(random.random()*h/2)),\
                        (w,h),(9,h)
            pygame.draw.polygon(self.image,pointlist)
            self.rect = self.image.get_rect()
            self.Vp = Vector()
...
class Block(pygame.sprite.Sprite):
    ...
    def setupImage(self):
        ...
        self.Vp.y = app.screen.get_height()/2.0
        self.delta = Vector()
...
class Lifebar(pygame.sprite.Sprite):
    def __init__(self,app,master):
        self.master = master
        self.groups = app.allgroup, app.bargroup
        self._layer = master._layer
        pygame.sprite.Sprite.__init__(self,self.groups)
        self.setupImage()
        self.oldpercent = 0
    def setupImage(self):
    ...
    def update(self,seconds):
        if self.master.health < 1:
            self.kill()
        self.percent = self.master.health/self.master.healthfull
...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self,app,layer=4):
        ...
        Lifebar(self.number)
        for _ in range(8):
            Fragment(self.Vp, True)
    def newSpeed(self):
        randomspeed = random.choice([-1,1])
        self.delta = Vector()
        self.delta.x = random.randint(self.birdspeedmin,self.birdspeedmax)
        self.delta.y = random.randint(self.birdspeedmin,self.birdspeedmax)
...
class Fragment(pygame.sprite.Sprite):
    def __init__(self,master,pos):
        self._layer = 9
        self.groups = master.app.allgroup, master.app.fraggroup
        pygame.sprite.Sprite.__init__(self,self.groups)
        self.target = pos
        self.Vp = pos
