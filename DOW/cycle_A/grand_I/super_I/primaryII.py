...
#------------------------------------#
#  primary II of superI(of GrandV)  #
#------------------------------------#
...
class Primary(ZigZag):
    ...
    def create_intermediates(self):
        intermediateA = Impulse (8875,8259,level=self.level-1,No=6)
        intermediateB = ZigZag  (8259,8580,level=self.level-1,No=7)
        intermediateC = Impulse(8580,8087,level=self.level-1,No=8)
...
primaryII = Primary(start=8875,end=8087,level=0,No=2,has_subwaves=True)
