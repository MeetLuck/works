...
#------------------------------------#
#  primary IV of superIV(of GrandV)  #
#------------------------------------#
...
class Primary(ZigZag):
    ...
    def create_intermediates(self):
        intermediateA = Impulse(17151,16333,level=self.level-1,No=6) #,has_subwaves=True)
        intermediateB = ZigZag (16333,17350,level=self.level-1,No=7) #,has_subwaves=True)
        intermediateC = Impulse(17350,15855,level=self.level-1,No=8)
...
primaryIV = Primary(start=17151,end=15855,level=0,No=4,has_subwaves=True)
