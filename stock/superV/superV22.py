...
class superV(Motive):
    ...
    def get_fibs(self):
        ...
        if self.i.complete:   fib20 = self.fib20()
        if self.ii.complete:  fib21 = self.fib(2,1)
        if self.iii.complete: fib31 = self.fib(3,1)
        if self.iv.complete:  fib43 = self.fib(4,3)
        if self.v.complete:   fib51 = self.fib(5,1) ; fib238 = self.fib238()

        print fg.GREEN + '\nFibonacci ratio for wave {wave_name}'.format(wave_name=self.name)
