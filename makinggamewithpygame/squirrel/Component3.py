...
def converToCameraPos(camera,obj):
...
class Grass:
    def __init__(self,camera):
        ...
        self.x,self.y = getRandomOffCameraPos(camera,self.width,self.height)
        self.rect = pygame.Rect( (self.x,self.y,self.width,self.height) )
    def draw(self,surface,camera):
        x,y = converToCameraPos(camera,self)
        grect = pygame.Rect( (x,y, self.width,self.height) ) 
...
class Squirrel:
    def draw(self,surface,camera):
        x,y = converToCameraPos(camera,self)
        pyrect = pygame.Rect(
                    (x, y - getBounceAmount(self.bounce, bouncerate, bounceheight),
...
#       #print self.x,camera.x,self.centerX
#       Squirrel.draw(surface,camera)
